  ...
  if (format.getLevelArrayTypes().size() < (size_t)format.getOrder()) {
    ...
    for (int i = 0; i < format.getOrder(); ++i) {
      ...
      if (modeType.getName() == Dense.getName()) {
        ...
        arrayTypes.push_back(Int32);
        arrayTypes.push_back(Int32);
      } else if (modeType.getName() == LgSparse.getName()) {
        arrayTypes.push_back(Rect(2));
        arrayTypes.push_back(Int32);
      } else if (modeType.getName() == LgSingleton.getName()) {
        arrayTypes.push_back(Int32);
      } else {
...
IndexStmt TensorBase::translateDistribution() {
  return this->content->tensorVar.translateDistribution();
}
