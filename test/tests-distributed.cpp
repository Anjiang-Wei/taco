...
TEST(distributed, legionSpMV) {
  ...
  auto add = [&](std::string name, std::function<IndexStmt(IndexStmt, Tensor<double> a, Tensor<double> B, Tensor<double> c, IndexExpr precomputed)> sched, Configuration conf, bool cuda = false) {
    ...
    // twice causes the transformation to not be successful.
    auto precomputed = B(i, j) * c(j);
    a(i) = precomputed;
    auto stmt = sched(a.getAssignment().concretize(), a, B, c, precomputed);
...
TEST(distributed, legionSpFormatConverterLib) {
  ...
  auto addConverter = [&](Format f, std::string formatName, bool assemble = false) {
    ...
    auto stmt = op.concretize();
    // Using assemble allows us to get more tensor assembly capabilities,
    // including the ability to transpose output tensors.
    if (assemble) {
      stmt = stmt.assemble(T.getTensorVar(), AssembleStrategy::Insert);
    }
    auto lowerPack = lowerLegionAssemble(stmt, name, true, true);
    if (assemble) {
      // TODO (rohany): Some code generated by assemble generates dangling
      //  variables and declarations that result in final output that does
      //  not compile. Simplifying the lowered code removes those problems.
      lowerPack = ir::simplify(lowerPack);
    }
