...
void top_level_task(const Task *task, const std::vector<PhysicalRegion> &regions, Context ctx, Runtime *runtime) {
  ...
  // TODO (rohany): Move this to an initialization in the library setup.
  ...
  std::shared_ptr<taco::LeafCallInterface> cugemm = std::make_shared<taco::CuGEMM>();
  {
    int gpus = runtime->select_tunable_value(ctx, Mapping::DefaultMapper::DEFAULT_TUNABLE_LOCAL_GPUS).get<size_t>();
    ...
    taco::ParallelUnit parUnit = gpus > 0 ? taco::ParallelUnit::DistributedGPU : taco::ParallelUnit::DistributedNode;
    taco::Grid m(gpus > 0 ? gpus : (omps > 0 ? omps : cpus));
    taco::DistVar x, y;
    taco::TensorDistributionNotation dist({x, y}, m, {x}, parUnit);
    ...
    stmt = stmt.concretize()
               .distribute({i}, {io}, {ii}, m, parUnit)
               .communicate({a(i, j), b(i, k), c(k, j)}, io)
	       ;
    if (gpus > 0) {
      stmt = stmt.swapLeafKernel(ii, cugemm);
    } else {
      stmt = stmt.swapLeafKernel(ii, gemm);
    }
  }
  // JIT the kernel.
...
int main(int argc, char** argv) {
  ...
  // TODO (rohany): Add more of this to some runtime setup functions.
