...
const char *ProcessorEnumName[] = {
    "CPU", "GPU", "IO", "PY", "PROC", "OMP", "ALLPROC",
};
...
const char *MemoryEnumName[] = {
    "SYSMEM", "FBMEM", "RDMEM", "ZCMEM", "SOCKMEM", "ALLMEM",
};
...
const char *APIName[] = {
    "SIZE",         "SPLIT",      "MERGE",         "SWAP",
    "SLICE",        "REVERSE",    "BALANCE_SPLIT", "AUTO_SPLIT",
    "GREEDY_SPLIT", "VOLUME",     "HAS",           "LEN",
    "TASKIPOINT",   "TASKISPACE", "TASKPARENT",    "TASKPROCESSOR",
};
...
const char *NodeTypeName[] = {
    "ProgramType",        "StmtType",          "ProcLstType",
    "ProcType",           "MemLstType",        "MemType",
    "ProcCustomType",     "RegionCustomType",  "ArgTypeType",
    "AssignType",         "IndexTaskMapType",  "ArgType",
    "ArgLstType",         "FuncDefType",       "BinaryExprType",
    "IdentifierExprType", "IdentifierLstType", "IntValType",
    "BoolValType",        "TupleExprType",     "SliceExprType",
    "FuncInvokeType",     "IndexExprType",     "NegativeExprType",
    "ExclamationType",    "APIType",           "TenaryExprType",
    "UnpackExprType",     "PrintType",         "PrintArgsType",
    "ReturnType",         "FuncStmtsType",     "ObjectInvokeType",
    "MSpaceType",         "TupleIntType",      "SetTupleIntType",
    "StarExprType",       "TaskNodeType",
};
...
class ProcCustomNode : public StmtNode {
  public:
    ...
    void print() {
        printf("ProcCustomNode %s %s\n", taskname.c_str(),
               ProcessorEnumName[proc_types[0]]);
    }
    Node *run();
};
...
class RegionCustomNode : public StmtNode {
  public:
    ...
    RegionCustomNode(const char *x, const char *y, ProcNode *z,
                     const MemLstNode *a) {
    ...
    void print() {
        printf("RegionCustomNode %s %s %s: %s as first\n", taskname.c_str(),
               region_name.c_str(), ProcessorEnumName[processor_type],
...
class AssignNode : public StmtNode {
  public:
    ...
    Node *run() {
        ...
        if (!(simplified->type == IntValType ||
              simplified->type == BoolValType ||
              simplified->type == MSpaceType ||
              simplified->type == TupleExprType)) {
            std::cout << "Cannot Assign " << NodeTypeName[simplified->type]
                      << std::endl;
            assert(false);
        }
...
class IdentifierLstNode : public Node {
  public:
    ...
    void append(const char *x) { idlst.push_back(std::string(x)); }
};
...
class IdentifierExprNode : public ExprNode {
  public:
    ...
    void print() { printf("IdentifierExprNode %s\n", name.c_str()); }
    Node *run() {
...
class IntValNode : public ExprNode {
  public:
    ...
    void print() { printf("%d", intval); }
    IntValNode *run() { return this; }
    IntValNode *negate() { return new IntValNode(-intval); }
...
class TupleIntNode : public ExprNode {
  public:
    ...
    ProcessorEnum
        final_proc; // validate mapping decision against dynamic machine model
...
class BoolValNode : public ExprNode {
  public:
    ...
    void print() { printf("BoolValNode: %s\n", boolval ? "true" : "false"); }
    Node *run() { return this; }
...
class ArgLstNode : public Node {
  public:
    ...
    ArgLstNode() { type = ArgLstType; }
    void print() {
        printf("ArgLstNode: %ld\n", arg_lst.size());
        for (size_t i = 0; i < arg_lst.size(); i++) {
            printf("Arg %ld: %s %s\n", i, ArgTypeName[arg_lst[i]->argtype],
                   arg_lst[i]->argname.c_str());
        }
    }
};
...
class TupleExprNode : public ExprNode {
  public:
    ...
    Node *run();
    ExprNode *Convert2TupleInt(
        bool allow_star =
            false); // if all nodes in exprlst are
                    // IntValNode(IntValType), then can be converted
  ...
  public:
    ...
    ForTupleExprNode(ExprNode *x, const char *y, ExprNode *z) {
        ...
        if ((z->type != TupleExprType && z->type != TupleIntType)) {
            std::cout
                << "For-comprehension must itererate over TupleExpr or TupleInt"
                << std::endl;
            assert(false);
        }
        range = z;
...
class LayoutCustomNode : public StmtNode {
  ...
  public:
    ...
    LayoutCustomNode(const char *x0, const char *x1, MemNode *x2,
                     ConstraintsNode *x3) {
...
class MemoryCollectNode : public StmtNode {
  ...
  public:
    ...
    void print() {
        printf("MemoryCollectNode %s %s", task_name.c_str(),
               region_name.c_str());
    }
    Node *run();
};
...
class TaskNode : public Node {
  public:
    ...
    void init(std::string name, std::vector<int> point,
    ...
    void print() {
        ...
        if (ispace != NULL)
            ispace->print();
        printf("parent:%s", parent != NULL ? parent->task_name.c_str()
                                           : "hasn't queried parent yet");
    }
...
class Tree2Legion {
  public:
    ...
    static std::unordered_map<std::string, std::vector<Processor::Kind>>
        task_policies;
    ...
    static std::unordered_set<std::pair<std::string, std::string>, HashFn1>
        memory_collect;

    static std::unordered_map<std::string,
                              std::unordered_map<Processor::Kind, int>>
        task2limit;
    ...
    bool should_fall_back(std::string task_name) {
        ...
        // std::cout << task_name << " will fallback for sharding/slicing,
        // warning!"
        // << std::endl;
        return true;
    }
    ...
    std::vector<Memory::Kind> query_memory_policy(std::string task_name,
        ...
        if (region_policies.count(key) > 0) {
            std::unordered_map<Processor::Kind, std::vector<Memory::Kind>>
                value = region_policies.at(key);
    ...
    std::vector<Memory::Kind>
    query_memory_list(std::string task_name,
        ...
        for (auto &region_name : region_names) {
            std::vector<Memory::Kind> to_append =
                query_memory_policy(task_name, region_name, proc_kind);
            res.insert(res.end(), to_append.begin(), to_append.end());
        }
        // task_name *
        std::vector<Memory::Kind> to_append2 =
            query_memory_policy(task_name, "*", proc_kind);
        res.insert(res.end(), to_append2.begin(), to_append2.end());
        // * region_name
        for (auto &region_name : region_names) {
            std::vector<Memory::Kind> to_append3 =
                query_memory_policy("*", region_name, proc_kind);
            res.insert(res.end(), to_append3.begin(), to_append3.end());
        }
        // * *
        std::vector<Memory::Kind> to_append4 =
            query_memory_policy("*", "*", proc_kind);
    ...
    ConstraintsNode *query_constraint_one_region(const std::string &task_name,
        ...
        if (layout_constraints.count(key) > 0) {
            std::unordered_map<Memory::Kind, ConstraintsNode *> value =
                layout_constraints.at(key);
    ...
    ConstraintsNode *
    query_constraint(const std::string &task_name,
                     const std::vector<std::string> &region_names,
                     const Memory::Kind &mem_kind) {
        // exact match first
        for (auto &region_name : region_names) {
            ConstraintsNode *res1 =
                query_constraint_one_region(task_name, region_name, mem_kind);
            if (res1 != NULL) {
                return res1;
            }
        }
        ConstraintsNode *res2 =
            query_constraint_one_region(task_name, "*", mem_kind);
        if (res2 != NULL) {
            return res2;
        }
        for (auto &region_name : region_names) {
            ConstraintsNode *res3 =
                query_constraint_one_region("*", region_name, mem_kind);
            if (res3 != NULL) {
                return res3;
            }
        }
...
std::unordered_map<std::string, std::vector<Processor::Kind>>
    Tree2Legion::task_policies;
using HashFn1 = PairHash<std::string, std::string>;
std::unordered_map<
    std::pair<std::string, std::string>,
    std::unordered_map<Processor::Kind, std::vector<Memory::Kind>>, HashFn1>
...
std::unordered_map<std::string, std::unordered_map<Processor::Kind, int>>
    Tree2Legion::task2limit;
...
std::unordered_set<std::pair<std::string, std::string>, HashFn1>
    Tree2Legion::memory_collect;
#endif
