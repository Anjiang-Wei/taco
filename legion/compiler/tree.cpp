...
std::vector<Processor::Kind>
MyProc2LegionProcList(std::vector<ProcessorEnum> myprocs) {
    std::vector<Processor::Kind> res;
    res.resize(myprocs.size());
    std::transform(myprocs.begin(), myprocs.end(), res.begin(),
                   MyProc2LegionProc);
    return res;
}
...
Node *RegionCustomNode::run() {
    std::pair<std::string, std::string> key =
        std::make_pair(taskname, region_name);
    ...
    if (Tree2Legion::region_policies.count(key) > 0) {
        ...
        std::unordered_map<Processor::Kind, std::vector<Memory::Kind>> value = {
            {proc, mem}};
...
Node *LayoutCustomNode::run() {
    std::pair<std::string, std::string> key =
        std::make_pair(task_name, region_name);
...
std::vector<std::vector<int>>pe)
  {
    ...
    if (proc_kind != Processor::NO_KIND) {
        if (MyProc2LegionProc(res2->final_proc) != proc_kind) {
            printf("%s is actually mapped to %s, but machine model is for %s",
                   task.c_str(), processor_kind_to_string(proc_kind).c_str(),
    ...
    printf("Must return TupleIntType or SetTupleIntType after invoking mapping "
           "function\n");
    assert(false);
}
return {};
...
Node *IndexTaskMapNode::run() {
    ...
    // IPoint x, ISpace y
    if (!(params.size() == 2 && params[0]->argtype == IPOINT &&
          params[1]->argtype == ISPACE)) {
        std::cout << "Entry mapping function's input must be (IPoint, ISpace)"
                  << std::endl;
        assert(false);
    }
    return NULL;
...
//         {
//           std::cout << "Number of elements of '!' should be the same" <<
...
//         else if (exclam_node->expr->type == TupleExprType)
//         {
//           TupleExprNode* tuple_expr_node = (TupleExprNode*)
//           exclam_node->expr; num_element_ = tuple_expr_node->exprlst.size();
//         }
//         else
//         {
//           std::cout << "Unsupported type after '!'" << std::endl;
...
//     FuncInvokeNode* func_invoke_node = new FuncInvokeNode(this->func_node,
//     func_args_node); tuple_res.push_back(func_invoke_node);
//   }
...
Node *FuncInvokeNode::run() {
    ...
    // functions for built-in objects (Machine Model)
        ...
        if (func_c->api == HAS) {
            ...
            Node *mspace_node =
                index_node->tuple->run(); // get the MSpace node from the
                                          // Identifier node's run()
            ...
            if (tuple_int_node_2->type == TupleExprType) {
                tuple_int_node_2 =
                    ((TupleExprNode *)tuple_int_node_2)->Convert2TupleInt();
            }
            assert(intnode_1->type == IntValType);
            if (tuple_int_node_2->type != TupleIntType) {
                printf("Autosplit's second argument must be tuple of integers, "
                       "while getting %s\n",
            ...
            return new MSpace(mspace_node, func_c->api, int_node_1->intval,
                              int_node_2->intval, int_node_3->intval);
        } else {
    ...
    // functions for built-in objects (Task Object)
        ...
        // todo: support task.processor(machine_model), return a point in
        // machine model space
    }
...
Node *BinaryExprNode::run() {
    ...
    if (simplified_left->type == TupleExprType) {
        simplified_left =
            ((TupleExprNode *)simplified_left)->Convert2TupleInt();
    }
    if (simplified_right->type == TupleExprType) {
        simplified_right =
            ((TupleExprNode *)simplified_right)->Convert2TupleInt();
    }
...
IntValNode *TupleIntNode::len() { return new IntValNode(tupleint.size()); }
...
Node *ObjectInvokeNode::run() {
    ...
    if (obj_tbd->type != MSpaceType && obj_tbd->type != TupleIntType) {
        std::cout << NodeTypeName[obj_tbd->type]
                  << " does not support volume/size" << std::endl;
        assert(false);
    }
    if (obj_tbd->type == TupleIntType) {
        ...
        // todo: should also support TaskNode: ipoint (get_point), ispace
        // (get_space), parent (get_parent);
    }
...
TupleExprNode *TupleExprNode::binary_op(TupleExprNode *right_op, BinOpEnum op) {
    ...
    for (int i = 0; i < exprlst.size(); i++) {
        if (!(exprlst[i]->type == IntValType &&
              right_op->exprlst[i]->type == IntValType)) {
            printf("Only IntValType inside TupleExprNode is allowed in binary "
                   "operation\n");
            assert(false);
        }
...
TupleIntNode *TupleIntNode::slice(int a, int b) {
    ...
    b = (b <= 0 ? tupleint.size() + b
                : b); // b == 0 means not slicing the right side
    std::vector<int> res;
    for (int i = a; i < b; i++) {
...
IntValNode *TupleIntNode::at(IntValNode *x) { return this->at(x->intval); }
...
Node *TupleExprNode::at(IntValNode *x) { return this->at(x->intval); }
    ...
    if (index_->type == TupleExprType) {
        ...
        if (int_node != NULL) // integer indexing
        {
            ...
            } else if (tuple_->type == MSpaceType) // dynamic machine model
            {
                ...
                printf("Unsupported IndexExprNode tuple's type: %s using "
                       "integer index\n",
        ...
        // index_node is a tuple of expression, dynamic machine model
        {
            ...
            if (converted->type == TupleIntType) {
                ...
                // get_node_proc returns a bool, indicating SetTupleInt or
                // TupleInt pass vector<int> and vector<vector<int>> into
                // get_node_proc as arguments
                std::vector<int> result1;
                std::vector<std::vector<int>> result2;
                bool is_result1 = machine_space->get_node_proc(
                    tuple_int_node->tupleint, result1, result2);
                if (is_result1) {
                    return new TupleIntNode(result1, machine_space->proc_type);
                } else {
                    return new SetTupleIntNode(result2,
                                               machine_space->proc_type);
                }
            } else {
                printf("Must only use tuple of integers to index a machine "
                       "model\n");
                assert(false);
            }
        } else {
        ...
        Node *right =
            slice_node->right == NULL ? NULL : slice_node->right->run();
        if ((!(left == NULL || left->type == IntValType)) ||
            (!(right == NULL || right->type == IntValType))) {
            printf(
                "Left/Right side of SliceExprNode must be NULL or integer\n");
            assert(false);
        }
        ...
        if (tuple_->type == TupleIntType) {
            ...
            return tuple_expr->slice(left_int, right_int);
                   ...
                   MSpaceType) // slicing a machine will return TupleInt
        {
            MSpace *machine = (MSpace *)tuple_;
            return (new TupleIntNode(machine->get_size()))
                ->slice(left_int, right_int);
        } else {
    ...
    for (size_t i = 0; i < exprlst.size(); i++) {
        ...
        if (simplified->type == UnpackExprType) {
            ...
            if (unpack_node->expr->type == TupleExprType) {
                ...
                TupleIntNode *tuple_int_node =
                    (TupleIntNode *)unpack_node->expr;
                for (size_t i = 0; i < tuple_int_node->tupleint.size(); i++) {
                    res->exprlst.push_back(
                        new IntValNode(tuple_int_node->tupleint[i]));
                }
            } else {
...
ExprNode *TupleExprNode::Convert2TupleInt(bool allow_star) {
    // if all nodes in std::vector<Node*> exprlst; are IntValNode(IntValType),
    // then can be converted to TupleIntNode
    ...
    for (int i = 0; i < this->exprlst.size(); i++) {
        ...
        } else if (this->exprlst[i]->type == StarExprType &&
                   allow_star == true) {
...
void push_local_symbol_with_top_merge(
    ...
    if (local_symbol.size() == 0) {
        ...
        std::unordered_map<std::string, Node *> current_top =
            local_symbol.top();
