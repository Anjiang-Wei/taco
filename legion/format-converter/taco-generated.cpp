...
void packLegionCOOToCSR(Context ctx, Runtime* runtime, LegionTensor* T, LegionTensor* TCOO) {
  ...
  DomainT<1> pDomT2 = runtime->get_index_space_domain(ctx, T2_pos.get_index_space());
  ...
  DomainT<1> pDomT20 = runtime->get_index_space_domain(ctx, T2_pos.get_index_space());
  int64_t csT2 = 0;
  for (int32_t pT200 = 0; pT200 < (pDomT20.bounds.hi[0] + 1); pT200++) {
...
void packLegionCOOToDSS(Context ctx, Runtime* runtime, LegionTensor* T, LegionTensor* TCOO) {
  ...
  DomainT<1> pDomT3 = runtime->get_index_space_domain(ctx, T3_pos.get_index_space());
...
void packLegionCOOToDDS(Context ctx, Runtime* runtime, LegionTensor* T, LegionTensor* TCOO) {
  ...
  DomainT<2> pDomT3 = runtime->get_index_space_domain(ctx, T3_pos.get_index_space());
  ...
  DomainT<2> pDomT30 = runtime->get_index_space_domain(ctx, T3_pos.get_index_space());
  int64_t csT3 = 0;
  for (int32_t pT300 = 0; pT300 < (pDomT30.bounds.hi[0] + 1); pT300++) {
    for (int32_t pT310 = 0; pT310 < (pDomT30.bounds.hi[1] + 1); pT310++) {
...
void packLegionCOOToDCSR(Context ctx, Runtime* runtime, LegionTensor* T, LegionTensor* TCOO) {
  ...
  DomainT<1> pDomT1 = runtime->get_index_space_domain(ctx, T1_pos.get_index_space());
  T1_pos_accessor[Point<1>(0)] = Rect<1>(0, -1);
  int32_t T1_crd_size = 1048576;
...
void packLegionCOOToSD(Context ctx, Runtime* runtime, LegionTensor* T, LegionTensor* TCOO) {
  ...
  while (iTCOO < pTCOO1_end) {
    ...
    if (T_capacity <= iT + 1) {
      int32_t T_vals_new_size = TACO_MAX(T_capacity * 2,((iT + 1) + 1));
