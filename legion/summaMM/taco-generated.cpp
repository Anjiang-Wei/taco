...
extern "C" void task_5(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  ...
  int64_t pointID3 = pointID2 * gridX + ko;
  b_dense_run_0 = runtime->get_index_subspace(ctx, b_dense_run_0_Partition_2, ko);
  c_dense_run_0 = runtime->get_index_subspace(ctx, c_dense_run_0_Partition_2, ko);
  ...
  for (PointInDomainIterator<1> itr = PointInDomainIterator<1>(domain); itr.valid(); itr++) {
    int64_t iln = (*itr)[0];
    Point<2> aStart = Point<2>((in * ((b1_dimension + (gridX - 1)) / gridX) + iln * (((b1_dimension + (gridX - 1)) / gridX + 1) / 2)), (jn * ((c2_dimension + (gridY - 1)) / gridY)));
    Point<2> aEnd = Point<2>(TACO_MIN((in * ((b1_dimension + (gridX - 1)) / gridX) + (iln * (((b1_dimension + (gridX - 1)) / gridX + 1) / 2) + (((b1_dimension + (gridX - 1)) / gridX + 1) / 2 - 1))), aDomain.hi()[0]), TACO_MIN((jn * ((c2_dimension + (gridY - 1)) / gridY) + ((c2_dimension + (gridY - 1)) / gridY - 1)), aDomain.hi()[1]));
    Rect<2> aRect = Rect<2>(aStart, aEnd);
    if (!aDomain.contains(aRect.lo) || !aDomain.contains(aRect.hi)) {
      aRect = aRect.make_empty();
    }
    aColoring[(*itr)] = aRect;
    Point<2> bStart = Point<2>((in * ((b1_dimension + (gridX - 1)) / gridX) + iln * (((b1_dimension + (gridX - 1)) / gridX + 1) / 2)), (ko * ((b2_dimension + (gridX - 1)) / gridX)));
    Point<2> bEnd = Point<2>(TACO_MIN((in * ((b1_dimension + (gridX - 1)) / gridX) + (iln * (((b1_dimension + (gridX - 1)) / gridX + 1) / 2) + (((b1_dimension + (gridX - 1)) / gridX + 1) / 2 - 1))), bDomain.hi()[0]), TACO_MIN((ko * ((b2_dimension + (gridX - 1)) / gridX) + ((b2_dimension + (gridX - 1)) / gridX - 1)), bDomain.hi()[1]));
    Rect<2> bRect = Rect<2>(bStart, bEnd);
    if (!bDomain.contains(bRect.lo) || !bDomain.contains(bRect.hi)) {
      bRect = bRect.make_empty();
    }
    bColoring[(*itr)] = bRect;
    Point<2> cStart = Point<2>((ko * ((b2_dimension + (gridX - 1)) / gridX)), (jn * ((c2_dimension + (gridY - 1)) / gridY)));
...
extern "C" void task_6(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  ...
  for (PointInDomainIterator<1> itr = PointInDomainIterator<1>(domain); itr.valid(); itr++) {
    int64_t ko = (*itr)[0];
    Point<2> bStart = Point<2>((in * ((b1_dimension + (gridX - 1)) / gridX)), (ko * ((b2_dimension + (gridX - 1)) / gridX)));
  ...
  for (PointInDomainIterator<1> itr = PointInDomainIterator<1>(domain); itr.valid(); itr++) {
    ...
    taskArgsRaw5.a_dense_run_0 = a_dense_run_0;
    ...
    taskArgsRaw5.b_dense_run_0_Partition_2 = b_dense_run_0_Partition_2;
    ...
    taskArgsRaw5.c_dense_run_0_Partition_2 = c_dense_run_0_Partition_2;
    taskArgsRaw5.gridX = gridX;
    taskArgsRaw5.gridY = gridY;
    taskArgsRaw5.in = in;
    taskArgsRaw5.jn = jn;
    taskArgsRaw5.ko = ko;
    taskArgsRaw5.pointID2 = pointID2;
    ...
    launcher.add_region_requirement(RegionRequirement(get_logical_region(runtime->get_logical_subregion_by_color(ctx, b_vals_partition, ko)), READ_ONLY, EXCLUSIVE, b_vals_parent, (0 | Mapping::DefaultMapper::VIRTUAL_MAP)).add_field(b_vals_field_id));
    launcher.add_region_requirement(RegionRequirement(get_logical_region(runtime->get_logical_subregion_by_color(ctx, c_vals_partition, ko)), READ_ONLY, EXCLUSIVE, c_vals_parent, (0 | Mapping::DefaultMapper::VIRTUAL_MAP)).add_field(c_vals_field_id));
...
extern "C" void task_7(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  ...
  int64_t iln = task->index_point[0];
...
extern "C" void task_8(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  ...
  launcher.add_region_requirement(RegionRequirement(runtime->get_logical_partition_by_color(ctx, get_logical_region(a_vals), pointID3), 0, READ_WRITE, EXCLUSIVE, a_vals_parent).add_field(a_vals_field_id));
  launcher.add_region_requirement(RegionRequirement(runtime->get_logical_partition_by_color(ctx, get_logical_region(b_vals), pointID3), 0, READ_ONLY, EXCLUSIVE, b_vals_parent).add_field(b_vals_field_id));
  launcher.add_region_requirement(RegionRequirement(runtime->get_logical_partition_by_color(ctx, get_logical_region(c_vals), pointID3), 0, READ_ONLY, EXCLUSIVE, c_vals_parent).add_field(c_vals_field_id));
...
extern "C" void task_9(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  ...
  for (PointInDomainIterator<1> itr = PointInDomainIterator<1>(domain); itr.valid(); itr++) {
    ...
    taskArgsRaw8.gridX = gridX;
    taskArgsRaw8.ko = ko;
    taskArgsRaw8.pointID2 = pointID2;
    ...
    launcher.add_region_requirement(RegionRequirement(get_logical_region(runtime->get_logical_subregion_by_color(ctx, runtime->get_logical_partition_by_color(ctx, get_logical_region(b_vals), pointID2), ko)), READ_ONLY, EXCLUSIVE, b_vals_parent, (0 | Mapping::DefaultMapper::VIRTUAL_MAP)).add_field(b_vals_field_id));
    launcher.add_region_requirement(RegionRequirement(get_logical_region(runtime->get_logical_subregion_by_color(ctx, runtime->get_logical_partition_by_color(ctx, get_logical_region(c_vals), pointID2), ko)), READ_ONLY, EXCLUSIVE, c_vals_parent, (0 | Mapping::DefaultMapper::VIRTUAL_MAP)).add_field(c_vals_field_id));
    launcher.tag = launcher.tag | TACOMapper::UNTRACK_VALID_REGIONS;
    launcher.tag = launcher.tag | TACOMapper::BACKPRESSURE_TASK;
    runtime->execute_task(ctx, launcher);
  }
