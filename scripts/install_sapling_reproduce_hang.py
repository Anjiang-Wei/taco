...
# python3 scripts/install_sapling_reproduce_hang.py --openmp --sockets 2 --cuda --dim 3 --multi-node --threads 20 --no-tblis
...
with pushd(args.distal_build_dir):
    ...
    run("./bin/taco-test", "--gtest_filter=distributed.cannonMM-cuda")
    run("make", "-j{}".format(args.threads), "cannonMM-cuda")
