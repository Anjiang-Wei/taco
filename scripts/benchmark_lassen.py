...
def main():
    ...
    for p in args.procs:
        ...
        prof_taco_cmd = ["-lg:prof", "1", "-lg:prof_logfile", f"tacoprof_{args.bench}_{p}_{args.gpus}_%.gz"]
        prof_dsl_cmd = ["-lg:prof", "1", "-lg:prof_logfile", f"dslprof_{args.bench}_{p}_{args.gpus}_%.gz"]
        ...
        if obcount_fix:
            taco_variant = taco_variant + ["-gex:obcount", str(16 * p)] # (4 + 2 * gpus/node) * nodes = 12 * nodes [fails for 8-node]
            dsl_variant = dsl_variant + ["-gex:obcount", str(16 * p)] # let's try 16 * nodes for Lassen
        if args.onlytaco:
